    ...
    lczero::Network *network, Tree *tree, int searchId)
{
    ...
    info.nodesCacheHits = info.nodesCreated - actualBatch.count();
...
#if defined(FIXES_FOR_HASH_CHANGE)
...
void SearchWorker::ensureRootAndChildrenScored()
{
...

    {
        ...
        for (int i = 0; i < childRefs->count(); ++i) {
            ...
            Node *child = root->generateEmbodiedChild(childRef);
...
void SearchEngine::startSearch()
{
    ...
    if (root->checkAndGenerateDTZ(&dtz)) {
        ...
        const Node *dtzNode = root->embodiedBestChild();
        Q_ASSERT(dtzNode);
    ...
    } else if (const Node *best = root->embodiedBestChild()) {
        ...
        if (const Node *ponder = best->embodiedBestChild())
        ...
        int pvDepth = 0;
        bool isTB = false;
        m_currentInfo.pv = root->principalVariation(&pvDepth, &isTB, false /*pin*/);
        float score = best->hasQValue() ? best->qValue() : -best->embodiedParent()->qValue();
...
void SearchEngine::receivedWorkerInfo(const WorkerInfo &info)
{
    ...
    Node *root = m_tree->embodiedRoot();
    const Node *best = root->embodiedBestChild();
    if (!best) {
        m_tree->treeMutex()->unlock();
        return;
    }
    ...
    Q_ASSERT(best);
    Q_ASSERT(best->hasEmbodiedParent());
